import { Component, OnInit, ViewChild } from '@angular/core';
import { ActivatedRoute } from '@angular/router';
import { AngularFirestore, AngularFirestoreCollection } from '@angular/fire/firestore';
import { Word, WordWithCounter } from '../../interfaces/words.interface';
import { NavController, Content } from '@ionic/angular';

@Component({
  selector: 'app-play',
  templateUrl: './play.page.html',
  styleUrls: ['./play.page.scss'],
})
export class PlayPage implements OnInit {
  @ViewChild(Content) public content: Content;
  public word: WordWithCounter;
  public answer: string;
  public categoryName: string;
  public status: string = 'default';
  public submitted: boolean = false;
  public correctWords: Word[] = [];
  public wrongWords: Word[] = [];
  public showResultPage: boolean = false;
  private index: number = 0;
  private words: WordWithCounter[];
  private wordsCollection: AngularFirestoreCollection<Word>;

  constructor(
    private route: ActivatedRoute,
    private db: AngularFirestore,
    private nav: NavController,
  ) {
    this.categoryName = this.route.snapshot.params.category;
    this.wordsCollection = db.collection('words', ref => ref.where('category', '==', this.categoryName));
    this.wordsCollection.valueChanges().subscribe(words => {
      this.shuffleWords(words);
      this.word = this.words[0];
    });
  }

  ngOnInit() {
  }

  onSubmit() {
    if (this.answer === '') {
      return false;
    } else {
      this.submitted = true;
    }

    if (this.answer === this.word.translate) {
      this.status = 'correct';
      if (this.word.hasOwnProperty('correct')) {
        this.word.correct++;
      } else {
        this.word.correct = 1;
      }
      // this.correctWords.push(this.word);
    } else {
      this.status = 'wrong';
      if (this.word.hasOwnProperty('wrong')) {
        this.word.wrong++;
      } else {
        this.word.wrong = 1;
      }
      // this.wrongWords.push(this.word);
    }
  }

  shuffleWords(words) {
    this.words = words.sort(function () {
      return .5 - Math.random();
    });
  }

  next() {
    if (this.index < this.words.length - 1) {
      this.index++;
    } else {
      this.shuffleWords(this.words);
      this.index = 0;
    }
    this.reset();
    this.word = this.words[this.index];
  }

  finish() {
    this.showResultPage = true;
    console.log(this.words);
    // this.content.resize();
    // this.nav.goBack();
  }

  reset() {
    this.status = 'default';
    this.submitted = false;
    this.answer = '';
  }

  restart() {
    this.resetResults();
    this.reset();
    this.index = 0;
    this.showResultPage = false;
  }

  resetResults() {
    this.words.forEach(word => {
      word.correct = 0;
      word.wrong = 0;
    });
  }

}
